...
void UCI::loop(int argc, char* argv[]) {
...
#ifdef IS_SOLVER
  if (argc < 2 || std::string(argv[1]) != "solver")
      return;
  const int i_start = 2;
#else
  const int i_start = 1;
  ...
  if (argc > i_start && (std::strcmp(argv[i_start], "noautoload") == 0))
  {
      cmd = "";
      argc = i_start;
  }
  else if (argc == i_start || !(std::strcmp(argv[i_start], "load") == 0))
  {
  ...
  do {
      if (argc == i_start && !getline(cin, cmd)) // Block here waiting for input or EOF
          cmd = "quit";
      ...
      else if (token == "load")     { load(is); argc = i_start; } // continue reading stdin
  ...
  } while (token != "quit" && argc == i_start); // Command line args are one-shot
}
