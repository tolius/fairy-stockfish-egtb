...
void MainThread::search() {
  ...
  std::cout << "bestmove " << UCI::move(rootPos, bestThread->rootMoves[0].pv[0]);
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (!rootNode && TB::Cardinality)
    {
        ...
        if (   piecesCount <= TB::Cardinality
            && !TB_Reader::tb_cache.empty()
            //&& pos.rule50_count() == 0
            && Options["UCI_Variant"] == "antichess"
            && !pos.can_castle(ANY_CASTLING))
        {
            int16_t val = egtb::NONE;
            uint8_t dtz = egtb::DTZ_NONE;
            bool is_err = TB_Reader::probe_EGTB(pos, val, dtz);
            ...
            if (!is_err) // && val != 0)
            {
                ...
                value = val == egtb::DRAW ? VALUE_DRAW
                    : val >= 0 ? VALUE_MATE - (int)val - ss->ply
                    : -VALUE_MATE - (int)val + ss->ply;
                Bound b = BOUND_EXACT;
        ...
        else if (TB::Cardinality && piecesCount <= TB::Cardinality
...
moves_loop: // When in check, search starts from here
  ...
  Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    if (ss->ply >= (Limits.mate ? Limits.mate * 2 : MAX_PLY))
